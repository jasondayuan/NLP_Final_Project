...
def train(args, train_dataset, model, tokenizer, eval_dataset=None):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(tqdm(epoch_iterator)):
            ...
            if args.model_type != "distilbert" and args.model_type != "t5" and args.model_type != "gpt2":
                inputs["token_type_ids"] = (
                    batch[2] if args.model_type in ["bert", "xlnet", "albert"] else None
                )  # XLM, DistilBERT, RoBERTa, GPT2, and XLM-RoBERTa don't use segment_ids
...
def main():
    ...
    parser.add_argument("--local-rank", "--local_rank", type=int, default=-1, help="For distributed training: local_rank")
    ...
    if "gpt2" in args.tokenizer_name:
    ...
    if args.change_positional_embedding_before_loading: # GPT2 Ignore
        expand_position_embeddings(model, args.max_length, args.model_name_or_path)

    if args.custom_weight is not None: # GPT2 Ignore
    ...
    if args.load_bert_weight is not None: # GPT2 Ignore
    ...
    if args.change_positional_embedding_after_loading: # GPT2 Ignore
        expand_position_embeddings(model, args.max_length, args.model_name_or_path)

    if args.nopooler: # GPT2 Ignore
        model.bert.pooler.apply(model._init_weights)

    if args.from_scratch: # GPT2 Ignore
